...
const BookAppointment = ({ selectedDate }) => {
    return (
        <section className='p-5'>
            <h2 className='heading'>Available Appointments on {selectedDate.toDateString()}</h2>
            <div className="row">
                {
                    bookingData.map(data => <BookingCard key={data._id} selectedDate={selectedDate} data={data} />)
                }
            </div>
        </section>
    );
